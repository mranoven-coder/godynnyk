...
- Додано кнопку «Встановити застосунок»: на Android/комп'ютері встановлює одним кліком, на iPhone показує підказку через Safari / Added an "Install app" button: installs with one click on Android/desktop, shows instructions via Safari on iPhone
- Інструкцію для батьків і вчителів оновлено відповідно до поточного функціоналу (тап по циферблату, уточнено спосіб встановлення) / Updated the parent/teacher guide to match current functionality (tap on the clock face, clarified how to install)

### v1.2 — 19.09.2026

- Додано розділ «Що нового» / «What's new» на сторінці, де публікуються зміни (💌 — зроблене за побажаннями) / Added a "What's new" section on the page where changes are published (💌 marks what was made following wishes)
...
- Додано англійську мову та перемикач UA / EN; переклад застосунку й інструкції для батьків / Added English, a UA / EN switcher and a translated guide
...
- Надійніше оновлення service worker (`cache: no-cache`, кеш `godynnyk-v3`) / More reliable service worker updates
- Додано авторство, ліцензію та номер версії / Added authorship, license and version number
